...
class CREATE(MSSQLStatement):
        ...
        def populate(schema, source, parts, alias):
            ...
            # join a collector
                ...
                if isinstance(collector, orb.Pipe):
                    ...
                    order = [
                        '`{0}`.`{1}`.`{2}` {3}'.format(join_schema.namespace() or default_namespace,
                                                       join_schema.dbname(),
                    ...
                    opts = {
                        ...
                        'namespace': '`{0}`'.format(join_schema.namespace() or default_namespace),
                        'model': '`{0}`'.format(join_schema.dbname()),
                        'through_namespace': '`{0}`'.format(pipe_schema.namespace() or default_namespace),
                    ...
                    if record_part in ('first', 'last'):
                        ...
                        join_table = '`{0}`.`{1}`'.format(join_schema.namespace() or default_namespace,
                                                          join_schema.dbname())
                else:
                    ...
                    opts = {
                        ...
                        'namespace': join_schema.namespace() or default_namespace,
    ...
    def _createTable(self, model, owner, includeReferences):
        ...
        if inherits:
            ...
            id_type = id_column.dbType('MSSQL').replace('AUTO_INCREMENT', '').strip()
            base_id_column = model.schema().idColumn()

            inherits = '`{0}` {1} REFERENCES `{2}`\n'.format(base_id_column.field(),
                                                             id_type,
                                                             inherits_model.schema().dbname())
            cmd_body.append(inherits)
        ...
        cmd  = 'CREATE TABLE IF NOT EXISTS `{namespace}`.`{table}` ({body});\n'
        cmd = cmd.format(namespace=model.schema().namespace() or default_namespace,
        ...
        if add_i18n:
            ...
            i18n_cmd  = 'CREATE TABLE IF NOT EXISTS `{namespace}`.`{table}_i18n` (\n'
